...
def image_entropy_torch(image):
    # 计算每个灰度级的像素频率
    hist = torch.histc(image, bins=256, min=0, max=255)
    pixel_prob = hist / image.numel()

    # 计算熵
    entropy = -torch.sum(pixel_prob * torch.log2(pixel_prob + 1e-10))  # 避免log(0)的情况
