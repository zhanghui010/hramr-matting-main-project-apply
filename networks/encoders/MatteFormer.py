...
class PAWSA(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None,
        ...
        for i in range(self.num_priorset):
            for j in range(3):
                relative_position_index_withPrior[:,
                num_patch + i * 3 + j:num_patch + i * 3 + j + 1] = last_idx + i * 3 + j
    ...
    def forward(self, x, mask=None, _inp_shape=None, prior_bins=None):
        ...
        for prior in prior_bins:
            ...
            uk_prior = uk_prior.expand((-1, (H // self.window_size[0]) * (W // self.window_size[1]), -1)).unsqueeze(
                2).contiguous().view(-1, 1, C)
            fg_prior = fg_prior.expand((-1, (H // self.window_size[0]) * (W // self.window_size[1]), -1)).unsqueeze(
                2).contiguous().view(-1, 1, C)
            bg_prior = bg_prior.expand((-1, (H // self.window_size[0]) * (W // self.window_size[1]), -1)).unsqueeze(
                2).contiguous().view(-1, 1, C)
            x = torch.cat((x, uk_prior, fg_prior, bg_prior), dim=1)

        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)] \
            .view(self.window_size[0] * self.window_size[1],
                  self.window_size[0] * self.window_size[1] + self.num_priorset * 3, -1)
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()

        # qkv projection for x
        qkv = self.qkv(x).reshape(B_, N + self.num_priorset * 3, 3, self.num_heads, C // self.num_heads).permute(2, 0,
                                                                                                                 3, 1,
                                                                                                                 4)
        ...
        if mask is not None:
            ...
            attn = attn.view(B_ // nW, nW, self.num_heads, N,
                             N + self.num_priorset * 3) + mask_withPrior.contiguous().unsqueeze(1).unsqueeze(0)
        ...
        prior_highway = self.qkv(prior_highway).reshape(B, self.num_priorset * 3, 3, self.num_heads,
...
class PASTBlock(nn.Module):
    def __init__(self, layer_idx, block_idx, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = PAWSA(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, use_prior=self.use_prior,
            num_priorset=self.num_priorset)
    ...
    def forward(self, x, mask_matrix, prior_memory=None, area_fg=None, area_bg=None, area_uk=None):
        ...
        attn_windows, priors_after_attn = self.attn(x_windows, mask=attn_mask, prior_bins=prior_bins,
                                                    _inp_shape=shifted_x.shape)
